...
/frontend    参考前端（Vite + vanilla JS + viem）
             当前：直连合约，完整兑换流程的参考实现
             未来：前端 JS 逻辑提炼为 @aastar/sdk 中的 MyShop 子包

/worker      必需的后端节点服务（非可选，但须可集成）
             提供协议运行所必需的三类服务：
               - permitServer：EIP-712 签名（SerialPermit / RiskAllowance）
               - apiServer：Shop/Item/Purchase 查询 API + 事件索引
               - watchPurchased：购后通知（Webhook / Telegram）
             设计目标：可作为独立模块集成到 AAStar 统一节点服务中
```

**Worker 的定位**：Worker 提供的服务是协议运行的必要支撑。合约只验证签名的有效性，但没有 permitServer，带门控的 item 无法被购买；没有 apiServer，前端无法发现 Shop 和 Item。Worker 将被集成进 AAStar 统一节点（`@aastar/node`），每个服务模块通过标准接口暴露，依赖通过 config 注入，不硬编码。
...
| Jury 仲裁 | 生态 JuryContract（生态级合约） | 纠纷可申诉，公正裁决 |
...
| ~~MetaMask 回退~~ | 不支持 MetaMask，产品路径是 AirAccount | — |
...
| **CommunityNFT** | 社区自部署（**TBD — 地址由社区配置，将由外部提供**） | 写（调用 mint） | 购买时 mint NFT 凭证给买家 | `ICommunityNFT.mint(address to, string uri, bool soulbound) → uint256 tokenId` |
...
| **EligibilityValidator** | 社区部署，**Protocol 维护白名单，Shop 只能选已批准合约** | 只读 validate() | 校验买家是否有购买资格（SBT 门控、声誉门控等）| M3+ Item 设置了资格门槛时 |
| **DisputeModule → 生态 JuryContract** | AAStar 生态级合约（非归属于任何单一模块） | 写 | DisputeModule 调用 JuryContract 创建投票任务，接收裁决回调 | M4 售后仲裁启用后 |
...
当前：frontend/src/main.js
    ...
    注意：不支持 MetaMask，产品路径是 AirAccount（M2），
    M1 阶段使用任意 EIP-1193 兼容钱包用于开发测试
...
IDisputeModule（争议模块接口——M4 新增合约）
  openDispute(purchaseId, evidenceUri) → disputeId
  resolveDispute(disputeId, score) ← 由生态 JuryContract 回调
  ← DisputeModule 负责：状态机 + 资金托管 + 裁决解释 + 释放
```
...
### 6.1 DisputeModule 与生态 JuryContract 的关系

**原则：MyShop 新增 DisputeModule.sol，直接使用生态 JuryContract 的投票能力，无需额外 adapter 层。**

生态 JuryContract 是 AAStar 生态级合约，服务整个生态，不归属于任何单一仓库。

```
MyShop 侧（新增）:
  DisputeModule.sol       争议生命周期状态机 + 资金托管 + 裁决执行
                          实现 IJuryCallback 接口，接收 JuryContract 回调

生态侧（直接调用，不 fork）:
  JuryContract.sol        陪审员注册、投票、共识计算、奖励分配（ERC-8004）
```

**DisputeModule 的职责（不是 Adapter）**：
- 争议状态机（opened → evidence_period → jurying → resolved）
- 争议资金托管：从 shop treasury 临时锁定，直至裁决
- 向 JuryContract 提交投票任务（包含证据 URI 和争议双方信息）
- 接收 JuryContract 回调的平均分 → 转换为业务决定（退款 / 不退款 / DAO 升级）
- 执行资金释放或退款
...
### 6.3 需要与生态 JuryContract 协调的接口问题

生态 JuryContract 当前设计以"任务/agent 验证"为主要场景，用于兑换纠纷需要确认：
- 是否支持通用仲裁任务类型（不绑定 agentId，改为绑定 purchaseId）
- `resolveDispute()` 回调接口是否标准化（`IJuryCallback` 接口）
- 陪审员奖励 token 是否统一使用 xPNTs / aPNTs，还是按 shop 配置
- 投票结果（平均分）的语义在纠纷场景下：≥50 分 = 支持买家（需确认）
